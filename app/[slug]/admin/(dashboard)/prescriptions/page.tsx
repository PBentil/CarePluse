...
const filters = [
    ...
    { label: "Paid", value: "paid" }, { label: "Dispensed", value: "dispensed" },
]
...
export default function HospitalPrescriptionsPage() {
    ...
    const fetchPrescriptions = useCallback(async () => {
        ...
        try {
            ...
            const res  = await fetch(`/api/${slug}/admin/prescriptions?${params}`)
    ...
    const columns: Column<Prescription>[] = [
        ...
        {
            header: "Drugs",
            accessor: (row) => <p className="text-xs text-zinc-600 dark:text-zinc-300">{row.items.map(i => i.drugName).join(", ")}</p>,
        },
        {
            header: "Total",
            accessor: (row) => <span className="text-xs font-medium text-zinc-900 dark:text-white">GH₵ {row.items.reduce((s, i) => s + i.price, 0).toFixed(2)}</span>,
        },
        {
            header: "Status",
        ...
        {
            header: "",
            accessor: (row) => <button onClick={() => setSelected(row)} className="text-xs text-primary hover:underline">View</button>,
            className: "w-16",
        },
    ]
    ...
    return (
        <div className="space-y-6">
            ...
            <DataTable data={prescriptions} columns={columns} loading={loading} emptyMessage="No prescriptions yet" />
                ...
                {selected && (
                    <div className="space-y-4">
                        <div className="flex items-center justify-between">
                            ...
                            <span className={`inline-flex items-center px-2.5 py-0.5 rounded-lg text-xs font-medium capitalize ${statusStyles[selected.status] ?? ""}`}>{selected.status}</span>
                        </div>
                        <div className="rounded-xl border border-zinc-100 dark:border-zinc-800 divide-y divide-zinc-50 dark:divide-zinc-800">
                            {selected.items.map(item => (
                                <div key={item.id} className="px-4 py-3 flex items-start justify-between">
                                    <div>
                                        <p className="text-sm font-medium text-zinc-900 dark:text-white">{item.drugName} <span className="text-zinc-400 font-normal">{item.dosage}</span></p>
                                        <p className="text-xs text-zinc-400">{item.frequency} · {item.duration}{item.notes && ` · ${item.notes}`}</p>
                                    </div>
                                    <p className="text-sm text-zinc-600 dark:text-zinc-300 ml-4">GH₵ {item.price.toFixed(2)}</p>
                                </div>
                            ))}
                            <div className="px-4 py-3 flex justify-between bg-zinc-50 dark:bg-zinc-800/50">
                                <p className="text-xs font-medium text-zinc-500">Total</p>
                                <p className="text-sm font-semibold text-zinc-900 dark:text-white">GH₵ {selected.items.reduce((s, i) => s + i.price, 0).toFixed(2)}</p>
                            </div>
                        </div>
                        <div className="flex gap-2">
                            {selected.status === "pending" && (
                                <button onClick={() => handleUpdate("paid")} disabled={saving} className="flex-1 px-4 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium disabled:opacity-50 transition-colors">Mark as paid</button>
                            )}
                            {selected.status === "paid" && (
                                <button onClick={() => handleUpdate("dispensed")} disabled={saving} className="flex-1 px-4 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium disabled:opacity-50 transition-colors">Mark dispensed</button>
                            )}
                        </div>
                    </div>
                )}
