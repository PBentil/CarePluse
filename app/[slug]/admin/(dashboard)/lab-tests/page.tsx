...
interface LabTest {
    id: string; testName: string; status: string; resultNotes?: string; resultUrl?: string; orderedAt: string
...
const filters = [
    { label: "All", value: "" }, { label: "Ordered", value: "ordered" },
...
export default function HospitalLabTestsPage() {
    ...
    const fetchLabTests = useCallback(async () => {
        ...
        try {
            ...
            const res  = await fetch(`/api/${slug}/admin/lab-tests?${params}`)
    ...
    const handleUpdate = async (status: string) => {
        ...
        setSaving(true)
        try {
            const res = await fetch(`/api/${slug}/admin/lab-tests/${selected.id}`, {
            ...
            setSelected(null); setResultNotes(""); setResultUrl("")
            fetchLabTests()
        } catch (error: unknown) {
            toast.error(error instanceof Error ? error.message : "Error")
    ...
    const columns: Column<LabTest>[] = [
        ...
        { header: "Test", accessor: (row) => <span className="text-xs text-zinc-700 dark:text-zinc-200">{row.testName}</span> },
        ...
        },
        {
            header: "",
            accessor: (row) => (
                <button onClick={() => { setSelected(row); setResultNotes(row.resultNotes ?? ""); setResultUrl(row.resultUrl ?? "") }} className="text-xs text-primary hover:underline">Update</button>
            ),
            className: "w-20",
        },
    ]
            ...
            <Modal open={!!selected} onClose={() => setSelected(null)} title="Update Lab Test" size="sm">
                {selected && (
                    <div className="space-y-4">
                        ...
                        <div><label className={labelClass}>Result notes</label><textarea value={resultNotes} onChange={e => setResultNotes(e.target.value)} placeholder="Describe findings..." rows={4} className={inputClass} /></div>
                        <div><label className={labelClass}>Result file URL</label><input value={resultUrl} onChange={e => setResultUrl(e.target.value)} placeholder="https://..." className={inputClass} /></div>
                        <div className="flex gap-2">
                            <button onClick={() => handleUpdate("processing")} disabled={saving} className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl border border-zinc-200 dark:border-zinc-700 text-sm text-zinc-600 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-zinc-800 disabled:opacity-50 transition-colors">
                                {saving && <Loader2 className="h-4 w-4 animate-spin" />} Mark processing
                            </button>
                            <button onClick={() => handleUpdate("completed")} disabled={saving} className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium disabled:opacity-50 transition-colors">
                                {saving && <Loader2 className="h-4 w-4 animate-spin" />} Upload results
                            </button>
                        </div>
                    </div>
                )}
