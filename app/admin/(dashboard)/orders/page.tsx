...
export default function AdminOrdersPage() {
    ...
    const columns: Column<Order>[] = [
        ...
        },
        {
            header: "Address",
            accessor: (row) => (
                <span className="text-xs text-zinc-600 dark:text-zinc-300">{row.deliveryAddress}</span>
            ),
        },
        {
            header: "Total",
        ...
        },
        {
            header: "Status",
            accessor: (row) => (
                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-lg text-xs font-medium ${statusStyles[row.status as OrderStatus]}`}>
                    {statusLabels[row.status as OrderStatus]}
                </span>
            ),
        },
        {
                    ...
                    <div className="space-y-4">
                        <div className="rounded-xl bg-zinc-50 dark:bg-zinc-800 p-4 space-y-2">
                            ...
                            <p className="text-xs font-medium text-zinc-700 dark:text-zinc-200">
                                GH₵ {selected.totalAmount.toFixed(2)}
                            </p>
                        </div>
                        ...
                        <div className="space-y-2">
                            {selected.status === "packed" && (
                                <button
                                    ...
                                    disabled={saving}
                                    className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium disabled:opacity-50 transition-colors"
                                >
                            ...
                            {selected.status === "on_the_way" && (
                                <button
                                    ...
                                    disabled={saving}
                                    className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium disabled:opacity-50 transition-colors"
                                >
                                    {saving && <Loader2 className="h-4 w-4 animate-spin" />}
                            ...
                            {selected.status === "delivered" && (
                                <p className="text-center text-sm text-zinc-400 py-2">Order completed</p>
                            )}
                        </div>
                    </div>
                )}
