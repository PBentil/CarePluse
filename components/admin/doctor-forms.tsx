...
export function DoctorForm({ doctor, onSuccess }: DoctorFormProps) {
    ...
    useEffect(() => {
        if (doctor) {
            reset({ name: doctor.name, specialty: doctor.specialty, email: doctor.email })
        }
    }, [doctor, reset])
    ...
    const onSubmit = async (data: FormValues) => {
        try {
            ...
            if (!res.ok) throw new Error(json.error || `Failed to ${isEdit ? "update" : "add"} doctor`)
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div>
                <label className={labelClass}>
                    Password{isEdit && <span className="normal-case font-normal text-zinc-400 ml-1">(leave blank to keep current)</span>}
                </label>
                <input
                    {...register("password")}
                    type="password"
