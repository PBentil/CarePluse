...
export function DataTable<T extends { id: string }>({
    ...
    const handleSearch = isControlled ? onSearchChange : setInternalSearch
    ...
    const showSearch = isControlled || searchKeys.length > 0
                        ...
                        filtered.map((row) => (
                            ...
                            >
                                {columns.map((col, j) => (
                                    <td
                                        key={j}
                                        className={`px-6 py-4 text-zinc-600 dark:text-zinc-300 ${col.className ?? ""}`}
                                    >
